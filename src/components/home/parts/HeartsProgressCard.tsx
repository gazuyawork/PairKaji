...
export default function HeartsProgressCard({
    ...
    const filledCount = Math.min(totalHearts, 10);        // ★ 5 → 10
    const extraCount = Math.max(totalHearts - 10, 0);     // ★ 5 → 10
    const hearts = useMemo(() => Array.from({ length: 10 }, (_, i) => i), []);

    const handleClick = () => {
        if (navigateTo) {
            const ts = Date.now();
            router.push(`${navigateTo}${navigateTo.includes('?') ? '&' : '?'}_t=${ts}`);
            return;
        }
        onClick?.();
    };
        ...
        >
            ...
            >
                ...
                <div className="flex items-center justify-between mb-2">
                    ...
                    {!!hintText && <p className="text-xs text-gray-400">{hintText}</p>}
                </div>
                            ...
                            >
                                <path
                                    ...
                                    style={
                                        isFilled
                                            ? undefined
                                            : { strokeDasharray: '3.2,3.2' } // 未充填時は点線風
                                    }
                                />
                            </motion.svg>
                        );
                ...
                {!isPaired && (
                    <p className="mt-3 text-xs text-gray-400">
                        ペア設定を行うと、相手からの「ありがとう（ハート）」がここにたまります。
                    </p>
                )}
            </div>
        </motion.div>
