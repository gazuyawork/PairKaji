...
// ✅ 数値設定（変更しやすいようにまとめた）
const SPIN_DURATION = 6; // ルーレット回転時間（秒）
const SHRINK_SCALE = 0; // 縮小後のスケール（例: 0.6 で 60%）
const SHRINK_DURATION = 2; // 縮小アニメーション時間（秒）
const MODAL_DELAY = 1800; // モーダル表示までの遅延（ミリ秒）
const MODAL_ANIMATION_DURATION = 2.5; // モーダルの表示アニメーション時間（秒）
...
>
  ...
  >
    ...
    <p className="text-2xl font-bold text-yellow-500">{OPTIONS[selectedIndex]}</p>
    <button
      onClick={(e) => {
        e.stopPropagation();
        setShowResult(false);
