...
export default function ClientLayout({ children }: { children: React.ReactNode }) {
  ...
  // ③ ★追加: /settings/line-link 判定（LINE通知設定ページ）
  const isSettingsLineLink = pathname?.startsWith('/settings/line-link') ?? false;

  // ④ ★変更: タッチを許可する画面に /settings/line-link を追加
  const allowTouch = isLanding || isProfile || isPricing || isSettingsLineLink;
  /* ▲▲▲ 今回の変更ここまで ▲▲▲ */
  ...
  return (
    <>
      ...
      {/* ★ 追加: /landing 入場時は key を切り替えて強制再マウント（初期化漏れ対策）
          ※ allowTouch のとき（/landing, /profile, /pricing, /settings/line-link）は 'allow-touch' を付与 */}
      <div
