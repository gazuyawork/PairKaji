...
// ✅ ビルド時の静的評価を強制的に無効化（page data collect で落ちるのを防ぐ）
...
export async function POST(req: NextRequest) {
  try {
    // ✅ ここで env を読む（import 時に例外が起きないようにする）
    ...
    // ✅ stripe シングルトン（import 時に env チェックで落ちる可能性がある）を使わず、
    //   リクエスト処理内で生成してビルド時評価を回避
    const stripe = new Stripe(STRIPE_SECRET_KEY);

    const parsed = await req.json().catch(() => null);
    const body: CreateCheckoutBody | null = isCreateCheckoutBody(parsed)
      ? parsed
      : null;
